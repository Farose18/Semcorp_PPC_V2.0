...
function logCsv(file, msg)
 ...
 if file1 == nil then
  fileName = filePath.."/"..anlagen_id.."_ZE_LOG_IM11_"..string.sub(now, 1, 10).."_11.csv"
  file = fileName
  file1 = io.open(file,"a")
  file1:write(anlagen_id..",SN:ZE_LOG_IMC11,ZE_LOG,0.0.0.0,3".."\n")
...
function logCsvZE(file, msg)
 ...
 if file2 == nil then
  fileNameZE = filePath.."/"..anlagen_id.."_ZE_LOG_IM12_"..string.sub(now, 1, 10).."_12.csv"
  file = fileNameZE
  file2 = io.open(file,"a")
  file2:write(anlagen_id..",SN:ZE_LOG_IMC12,ZE_LOG,0.0.0.0,4".."\n")
  -- log format ts, dg01Pac, dg02Pac, pvPac, pacLimit, gridConnSt, pacLimitSet, gridConnSet
...
function logCsvZEU(file, msg)
 ...
 if file3 == nil then
  fileNameZEU = filePath.."/"..anlagen_id.."_ZE_LOG_IM13_"..string.sub(now, 1, 10).."_13.csv"
  file = fileNameZEU
  file3 = io.open(file,"a")
  file3:write(anlagen_id..",SN:ZE_LOG_IMC13,ZE_LOG,0.0.0.0,5".."\n")
  -- log format ts, dg01Pac, dg02Pac, pvPac, pacLimit, gridConnSt, pacLimitSet, gridConnSet
...
function logCsvZEF(file, msg)
   ...
   if file4 == nil then
      fileNameZEF = filePath.."/"..anlagen_id.."_ZE_LOG_IM14_"..string.sub(now, 1, 10).."_14.csv"
      file = fileNameZEF
      file4 = io.open(file,"a")
      file4:write(anlagen_id..",SN:ZE_LOG_IMC14,ZE_LOG,0.0.0.0,4".."\n")
      -- log format ts, dg01Pac, dg02Pac, pvPac, pacLimit, gridConnSt, pacLimitSet, gridConnSet
...
function logCsvZEPF(file, msg)
   ...
   if file5 == nil then
      fileNameZEPF = filePath.."/"..anlagen_id.."_ZE_LOG_IM15_"..string.sub(now, 1, 10).."_15.csv"
      file = fileNameZEPF
      file5 = io.open(file,"a")
      file5:write(anlagen_id..",SN:ZE_LOG_IMC15,ZE_LOG,0.0.0.0,5".."\n")
      -- log format ts, dg01Pac, dg02Pac, pvPac, pacLimit, gridConnSt, pacLimitSet, gridConnSet
...
if (dev == lastDev) then
 if ampZE01FUHF == nil then
  ...
  ampZE01FUHF =
  function(dev)
    local pacMaxSetCmd = (WR.read(dev, "PAC_MAX_SET_CMD")/1.285)
    local qacMaxSetCmd = (WR.read(dev, "QAC_MAX_SET_CMD")/1.285)
    local blkPacMaxCmd = (WR.read(dev, "BLOCK_PAC_MAX_CMD")/1.285)
    local blkQacMaxCmd = (WR.read(dev, "BLOCK_QAC_MAX_CMD")/1.285)
    ...
    if is_nan(pacMaxSetCmd) then pacMaxSetCmd = 6300 end
    if is_nan(qacMaxSetCmd) then qacMaxSetCmd = 3780 end
    if is_nan(blkPacMaxCmd) then blkPacMaxCmd = 6300 end
    if is_nan(blkQacMaxCmd) then blkQacMaxCmd = 3780 end
   ...
   local zePac = (WR.read(dev, "ZE_PAC") * (6.30/(21.150))) * 1000
   if is_nan(zePac) then zePac = 6300 end
   ...
   local zePMinLoad = (WR.read(dev, "PAC_PC_CMD") * (6.30/(21.150))) * 1000
   local zePCriticalLoad = zePMinLoad - ((pacLLSetCmd / 100) * blkPacMaxCmd)

   local zeMinLoad = (WR.read(dev, "QAC_PC_CMD") * (6.30/(21.150))) * 1000
...
--[[
   ...
   local zeQac = ((WR.read(dev, "ZE_QAC")) * (6.30/(21.150))) * 1000
...
--[[
   ...
   local inv3_Com = WR.read("SN:B02_INV01", "COMMUNICATION_STATUS")
   --local inv6_Com = WR.read("SN:B03_INV02", "COMMUNICATION_STATUS")


   local inv3_Pac = WR.read("SN:B02_INV01", "PAC")
   ...
   WR.setProp("SN:B02_INV01", "TOTAL_INV_ONLINE", totalInverters)
   --WR.setProp("SN:B03_INV02", "TOTAL_INV_ONLINE", totalInverters)

   local totalQInverters = 0
   if (inv3_Com < 1)  then totalQInverters = totalQInverters + 1 end
   --if (inv6_Com < 1)  then totalQInverters = totalQInverters + 1 end


   WR.setProp("SN:B02_INV01", "TOTAL_INV_ONLINE_Q", totalQInverters)
...
if ((not(is_nan(zeFac))) and (PowSelCmd == 1) and (totalInverters > 0) and (fOnOffCmd == 1) and (facOnOffCmd == 1)) then -- facOnOffCmd
 ...
 -- pacLimit = ((((blkPacMaxCmd + (((-1 * (zeFac - zeFMinLoad)  / (zeFMinLoad ))* 100))/fDroopCmd)* blkPacMaxCmd)) / totalInverters) / ((100 - facGRSetCmd) / 100)
            ...
            elseif ((zeFac < (zeFMinLoad - zeFThreshold)) and (zeFac >= (zeFMinLoad - zeFCriticalLoad)) and (minPacLimit <= (pacMaxSetCmd+1)) and (zeIac < zeIacSetCmd)) then
              olddelF = delF
							delF = zeFac - zeFMinLoad
							local diffdelF = delF - olddelF
							if (diffdelF ~= 0) then
            ...
            elseif ((zeFac >= (zeFMinLoad - zeFLCriticalLoad)) and ((zeFac < (zeFMinLoad - zeFCriticalLoad)) and (minPacLimit <= (pacMaxSetCmd+1)) and (zeIac < zeIacSetCmd))) then         ----less than changed to Greater than
                 olddelF = delF
				 delF = zeFac - zeFMinLoad
            ...
            elseif ((zeFac <= (zeFMinLoad + zeFUCriticalLoad)) and ((zeFac > (zeFMinLoad + zeFCriticalLoad)) and (minPacLimit <= (pacMaxSetCmd+1)) and (zeIac < zeIacSetCmd))) then         ----less than changed to Greater than
                 olddelF = delF
				 delF = zeFac - zeFMinLoad
                 diffdelF = delF - olddelF
                 if (diffdelF ~= 0) then
            ...
            elseif ((zeIac >= zeIacSetCmd) and (maxPacLimit <= (pacMaxSetCmd+1))) then
               if (zeIac > zeIacMaxCmd) then
            ...
            elseif ((zeFac <= (zeFMinLoad + zeFThreshold)) and (zeFac >= (zeFMinLoad - zeFThreshold)) and (maxPacLimit <= (pacMaxSetCmd+1)) and (zeIac < zeIacSetCmd)) then         ----less than changed to Greater than
