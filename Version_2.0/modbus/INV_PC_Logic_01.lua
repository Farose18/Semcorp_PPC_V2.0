...
function logCsv(file, msg)
 ...
 if file1 == nil then
  fileName = filePath.."/"..anlagen_id.."_ZE_LOG_IM01_"..string.sub(now, 1, 10).."_01.csv"
  file = fileName
...
function logCsvZE(file, msg)
 ...
 if file2 == nil then
  fileNameZE = filePath.."/"..anlagen_id.."_ZE_LOG_IM02_"..string.sub(now, 1, 10).."_02.csv"
  file = fileNameZE
...
function logCsvZEU(file, msg)
 ...
 if file3 == nil then
  fileNameZEU = filePath.."/"..anlagen_id.."_ZE_LOG_IM03_"..string.sub(now, 1, 10).."_03.csv"
  file = fileNameZEU
...
function logCsvZEF(file, msg)
   ...
   if file4 == nil then
      fileNameZEF = filePath.."/"..anlagen_id.."_ZE_LOG_IM04_"..string.sub(now, 1, 10).."_04.csv"
      file = fileNameZEF
      file4 = io.open(file,"a")
      file4:write(anlagen_id..",SN:ZE_LOG_IMC04,ZE_LOG,0.0.0.0,4".."\n")
      -- log format ts, dg01Pac, dg02Pac, pvPac, pacLimit, gridConnSt, pacLimitSet, gridConnSet
...
function logCsvZEPF(file, msg)
   ...
   if file5 == nil then
      fileNameZEPF = filePath.."/"..anlagen_id.."_ZE_LOG_IM05_"..string.sub(now, 1, 10).."_5.csv"
      file = fileNameZEPF
      file5 = io.open(file,"a")
      file5:write(anlagen_id..",SN:ZE_LOG_IMC05,ZE_LOG,0.0.0.0,5".."\n")
      -- log format ts, dg01Pac, dg02Pac, pvPac, pacLimit, gridConnSt, pacLimitSet, gridConnSet
...
if (dev == lastDev) then
 if ampZE01FUHF == nil then
  ...
  function(dev)
    ...
    local fDroopCmd = WR.read(dev, "FAC_DROOP_SET_CMD")


    if is_nan(pacMaxSetCmd) then pacMaxSetCmd = 8100 end
    if is_nan(qacMaxSetCmd) then qacMaxSetCmd = 4860 end
    if is_nan(blkPacMaxCmd) then blkPacMaxCmd = 8100 end
    if is_nan(blkQacMaxCmd) then blkQacMaxCmd = 4860 end
   ...
   local zePac = (WR.read(dev, "ZE_PAC") * (8.1/(21.150))) * 1000
   if is_nan(zePac) then zePac = 8100 end
...
--[[
   ...
   local zeQac = ((WR.read(dev, "ZE_QAC")) * (8.1/(21.150))) * 1000
...
--[[
   ...
   local inv1_Com = WR.read("SN:B01_INV01", "COMMUNICATION_STATUS")
   --local inv2_Com = WR.read("SN:B01_INV02", "COMMUNICATION_STATUS")

   local inv1_Pac = WR.read("SN:B01_INV01", "PAC")
   --local inv2_Pac = WR.read("SN:B01_INV02", "PAC")
   ...
   WR.setProp("SN:B01_INV01", "TOTAL_INV_ONLINE", totalInverters)
   --WR.setProp("SN:B01_INV02", "TOTAL_INV_ONLINE", totalInverters)
   
   local totalQInverters = 0
   if (inv1_Com < 1) then totalQInverters = totalQInverters + 1 end
   --if (inv2_Com < 1) then totalQInverters = totalQInverters + 1 end

   WR.setProp("SN:B01_INV01", "TOTAL_INV_ONLINE_Q", totalQInverters)
   --WR.setProp("SN:B01_INV02", "TOTAL_INV_ONLINE_Q", totalQInverters)
